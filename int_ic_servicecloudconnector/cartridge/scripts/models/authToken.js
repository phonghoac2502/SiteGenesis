...
function isValidAuth() {
    if (!this.token || !this.token.access_token) { // If we didn't load any token yet, load it from cache (Custom object)
        ...
        if (!cachedToken.expires || Date.now() >= (cachedToken.expires - EXPIRE_LIMIT)) {
            this.token = refreshToken(cachedToken);
        } else {
...
function getValidToken() {
    if(!this.isValidAuth()){
        ...
        var username = curSite.getCustomPreferenceValue('sscUsername');
        var password = curSite.getCustomPreferenceValue('sscPassword');
        if (empty(username) || empty(password)) {
            throw new Error('Service preferences requires valid Service Cloud username and password (different from client ID and secret)');
        }
        var svc = RestService.get(RestService.serviceIDs.auth);
        svc.addParam('grant_type', 'password');
        svc.addParam('username', username);
        svc.addParam('password', password);
        var result = svc.call();
 ...
 * @alias module:models/authToken~AuthToken#getValidToken
 */
function refreshToken(cachedToken) {
    var svc = RestService.get(RestService.serviceIDs.auth);
    svc.addParam('grant_type', 'refresh_token');
    svc.addParam('refresh_token', cachedToken.access_token);
    var result = svc.call();
    if (result.status === 'OK' && result.object) {
        this.token = updateCachedTokenObject(result.object);
    }else{
